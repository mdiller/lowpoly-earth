...
// because it broke as a #define for some reason
...
GLuint	GlobeList;				// object display list
...
bool    mode_is_outside;
bool    mode_is_paused;
float   animation_cycle_percent;
int     time_spent_paused;
...
{
	mode_is_outside = true;
	mode_is_paused = false;
	animation_cycle_percent = 0;
	pause_start_time = 0;
	time_spent_paused = 0;
...
Animate()
{
	int ms = (glutGet(GLUT_ELAPSED_TIME) - time_spent_paused) % MS_PER_CYCLE;

	// Sets heli blade angle in degrees
	animation_cycle_percent = ((float)ms / MS_PER_CYCLE);
...
void DoViewMenu( int is_outside )
{
	mode_is_outside = is_outside == 1;
...
// ranges are 0 to 1 for each value
struct ColorRGB {
	float r;
	float g;
	float b;
...
ColorRGB ColorFromPercent(float percent) {
	float n = percent * 3;
	ColorRGB color;
	color.r = 0;
	color.g = 0;
...
ColorRGB ColorFromRadians(float radians) {
	while (radians >= (2 * PI)) {
		radians -= 2 * PI;
	}
	while (radians < 0) {
	...
	return ColorFromPercent(radians / (2 * PI));
}

void drawTriangle(Point point1, Point point2, Point point3) {
	// ColorRGB color = ColorFromRadians(point1.radians);
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
		...
		case 'm':
		case 'M':
			mode_is_outside = !mode_is_outside;
			break;

		case 'p':
		case 'P':
			mode_is_paused = !mode_is_paused;
			if (mode_is_paused) {
				pause_start_time = glutGet(GLUT_ELAPSED_TIME);
				glutIdleFunc(NULL);
			}
			else {
				time_spent_paused += glutGet(GLUT_ELAPSED_TIME) - pause_start_time;
				glutIdleFunc(Animate);
			}
			break;
